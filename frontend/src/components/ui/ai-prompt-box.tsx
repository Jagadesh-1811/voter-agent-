...
// Inject styles into document (safe for SSR/Next.js)
...
const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
  ...
  const bars = React.useMemo(() => {
    // Generate static values to avoid impurity during render
    return [...Array(visualizerBars)].map((_, i) => ({
      height: `${Math.max(15, ((i * 33) % 100))}%`,
      duration: `${0.5 + ((i * 13) % 50) / 100}s`,
    }));
  }, [visualizerBars]);
                ...
                <AnimatePresence>
                  {showThink && (
                    <motion.span
                      ...
                      exit={{ width: 0, opacity: 0 }}
                      transition={{ duration: 0.2 }}
                      className="text-xs overflow-hidden whitespace-nowrap text-violet-500 shrink-0"
                    >
                      Think
                    </motion.span>
                  )}
